...
def busqueda_amplitud(orig, dest, plot=False):
...
def busqueda_profundidad(orig, dest, plot=False):
    # Inicialización de los nodos del grafo
    for node in G.nodes:
...
def busqueda_limitada_profundidad(orig, dest, limite, plot=False):
    # Inicialización de los nodos del grafo
    for node in G.nodes:
        G.nodes[node]["visited"] = False  # Marcando todos los nodos como no visitados
        G.nodes[node]["previous"] = None  # Estableciendo el nodo anterior como None
...
def busqueda_costo_uniforme(orig, dest, plot=False):
    ...
    while pq:  # Mientras la cola de prioridad no esté vacía
        ...
        for edge in G.out_edges(node):  # Iterando sobre los bordes salientes del nodo
            ...
            if G.nodes[neighbor]["distance"] > G.nodes[node]["distance"] + weight:  # Si la distancia al vecino es mayor que la suma de la distancia al nodo actual y el peso del borde
                G.nodes[neighbor]["distance"] = G.nodes[node]["distance"] + weight  # Actualizando la distancia al vecino
                G.nodes[neighbor]["previous"] = node  # Estableciendo el nodo actual como el nodo anterior del vecino
                heapq.heappush(pq, (G.nodes[neighbor]["distance"], neighbor))  # Agregando el vecino a la cola de prioridad con su nueva distancia como prioridad
...
def busqueda_bidireccional(orig, dest, plot=False):
    ...
    for node in G.nodes:
        G.nodes[node]["visited_forward"] = False  # Marcando todos los nodos como no visitados en la búsqueda hacia adelante
        G.nodes[node]["visited_backward"] = False  # Marcando todos los nodos como no visitados en la búsqueda hacia atrás
        G.nodes[node]["previous_forward"] = None  # Estableciendo el nodo anterior como None en la búsqueda hacia adelante
        G.nodes[node]["previous_backward"] = None  # Estableciendo el nodo anterior como None en la búsqueda hacia atrás
        G.nodes[node]["distance_forward"] = float("inf")  # Estableciendo la distancia a cada nodo como infinito en la búsqueda hacia adelante
        G.nodes[node]["distance_backward"] = float("inf")  # Estableciendo la distancia a cada nodo como infinito en la búsqueda hacia atrás
        G.nodes[node]["size"] = 0  # Estableciendo el tamaño del nodo como 0
    ...
    G.nodes[orig]["distance_forward"] = 0  # Estableciendo la distancia al nodo de origen como 0 en la búsqueda hacia adelante
    G.nodes[dest]["distance_backward"] = 0  # Estableciendo la distancia al nodo de destino como 0 en la búsqueda hacia atrás
    G.nodes[orig]["size"] = 50  # Estableciendo el tamaño del nodo de origen como 50
    G.nodes[dest]["size"] = 50  # Estableciendo el tamaño del nodo de destino como 50

    # Colas de prioridad para las búsquedas hacia adelante y hacia atrás
    pq_forward = [(0, orig)]  # Cola de prioridad para la búsqueda hacia adelante con el nodo de origen y su distancia como prioridad
    pq_backward = [(0, dest)]  # Cola de prioridad para la búsqueda hacia atrás con el nodo de destino y su distancia como prioridad
    ...
    while pq_forward or pq_backward:  # Mientras alguna de las colas de prioridad no esté vacía
        ...
        if pq_forward:
            _, node_forward = heapq.heappop(pq_forward)  # Sacando el nodo con la menor distancia de la cola hacia adelante
            
            if G.nodes[node_forward]["visited_backward"]:  # Si el nodo hacia adelante ya ha sido visitado por la búsqueda hacia atrás
                if plot:
                    print("Iteraciones:", step)  # Imprimiendo el número de iteraciones si se establece plot como True
                    plot_graph()  # Visualizando el grafo si se establece plot como True
                return  # Terminando la función
            
            if G.nodes[node_forward]["visited_forward"]:  # Si el nodo hacia adelante ya ha sido visitado por la búsqueda hacia adelante, continuamos con el siguiente nodo en la cola
                continue
            
            G.nodes[node_forward]["visited_forward"] = True  # Marcando el nodo como visitado en la búsqueda hacia adelante
            ...
            for edge in G.out_edges(node_forward):  # Iterando sobre los bordes salientes del nodo hacia adelante
                ...
                if G.nodes[neighbor]["distance_forward"] > G.nodes[node_forward]["distance_forward"] + weight:  # Si la distancia al vecino desde el nodo de origen es mayor que la suma de la distancia al nodo actual y el peso del borde
                    G.nodes[neighbor]["distance_forward"] = G.nodes[node_forward]["distance_forward"] + weight  # Actualizando la distancia al vecino desde el nodo de origen
                    G.nodes[neighbor]["previous_forward"] = node_forward  # Estableciendo el nodo actual como el nodo anterior del vecino en la búsqueda hacia adelante
                    heapq.heappush(pq_forward, (G.nodes[neighbor]["distance_forward"], neighbor))  # Agregando el vecino a la cola de prioridad hacia adelante con su nueva distancia como prioridad
                    for edge2 in G.out_edges(neighbor):  # Iterando sobre los bordes salientes del vecino
                        style_active_edge((edge2[0], edge2[1], 0))  # Aplicando el estilo para un borde activo
        
        # Búsqueda hacia atrás
        if pq_backward:
            _, node_backward = heapq.heappop(pq_backward)  # Sacando el nodo con la menor distancia de la cola hacia atrás
            
            if G.nodes[node_backward]["visited_forward"]:  # Si el nodo hacia atrás ya ha sido visitado por la búsqueda hacia adelante
                if plot:
                    print("Iteraciones:", step)  # Imprimiendo el número de iteraciones si se establece plot como True
                    plot_graph()  # Visualizando el grafo si se establece plot como True
                return  # Terminando la función
            
            if G.nodes[node_backward]["visited_backward"]:  # Si el nodo hacia atrás ya ha sido visitado por la búsqueda hacia atrás, continuamos con el siguiente nodo en la cola
                continue
            
            G.nodes[node_backward]["visited_backward"] = True  # Marcando el nodo como visitado en la búsqueda hacia atrás
            ...
            for edge in G.in_edges(node_backward):  # Iterando sobre los bordes entrantes del nodo hacia atrás
                ...
                if G.nodes[neighbor]["distance_backward"] > G.nodes[node_backward]["distance_backward"] + weight:  # Si la distancia al vecino desde el nodo de destino es mayor que la suma de la distancia al nodo actual y el peso del borde
                    G.nodes[neighbor]["distance_backward"] = G.nodes[node_backward]["distance_backward"] + weight  # Actualizando la distancia al vecino desde el nodo de destino
                    G.nodes[neighbor]["previous_backward"] = node_backward  # Estableciendo el nodo actual como el nodo anterior del vecino en la búsqueda hacia atrás
                    heapq.heappush(pq_backward, (G.nodes[neighbor]["distance_backward"], neighbor))  # Agregando el vecino a la cola de prioridad hacia atrás con su nueva distancia como prioridad
...
def reconstruct_path(orig, dest, plot=False, algorithm=None):
    ...
    while curr != orig:  # Mientras no lleguemos al nodo de origen
        ...
        speeds.append(G.edges[(prev, curr, 0)]["maxspeed"])  # Añadiendo la velocidad del borde a la lista de velocidades
    ...
    if plot:  # Si se solicita visualización
        # Imprimiendo la distancia total, velocidad promedio y tiempo total del recorrido
        print(f"Distance: {dist}")
        print(f"Avg. speed: {sum(speeds)/len(speeds)}")
        print(f"Total time: {dist/(sum(speeds)/len(speeds)) * 60}")
        print("Grafito")
        print(obtener_coordenadas(G))
        plot_graph()  # Visualizando el grafo
...
busqueda_amplitud(start, end, plot=True)
reconstruct_path(start, end, plot=True)

print("----- Busqueda por profundidad -----")
busqueda_profundidad(start, end, plot=True)
reconstruct_path(start, end, plot=True)

print("----- Busqueda limitada por profundidad -----")
limite_profundidad = 300
busqueda_limitada_profundidad(start, end, limite_profundidad, plot=True)
reconstruct_path(start, end, plot=True)

print("----- Busqueda de costo uniforme -----")
busqueda_costo_uniforme(start, end, plot=True)
reconstruct_path(start, end, plot=True)

print("----- Busqueda bidireccional -----")
busqueda_bidireccional(start, end, plot=True)
reconstruct_path(start, end, plot=True)
